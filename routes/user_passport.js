                ...
                database.UserModel.findOne({email:req.user.email}).populate('reservationlist', 'title author updated_at').exec(function (err, results) {
					...
					fs.readdir(directory, function(err, filelist){
						...
						if(filelist.includes(results.profile_path)){
							var PROGILE_imgpath = 'uploads/'+results.profile_path;
						}else{
							var PROGILE_imgpath = '/public/images/pic05.png';
						}
						
						res.render('profile.ejs', {user: req.user,  admin:req.user.admin, posts:results, login_success: true, profile:PROGILE_imgpath});
					});
