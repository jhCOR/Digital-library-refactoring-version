var plusFriends = (req, res) => {
	...
	if (database.db) {
		...
		database.UserModel.find({ email: addfriend }).countDocuments().exec(async function (err, count) {
				if (count > 0) {
					database.UserModel.findByIdAndUpdate(req.user._id,	{ $addToSet: { friends: addfriend} },function (err, theresult) {
							if (err) {
