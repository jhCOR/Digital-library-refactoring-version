...
var insertAnnouncement = async function (req, res) {
	...
	if(req.user){
		...
		var announceForMainPage = new databaseModel(req, res);
		announceForMainPage.checkAndSkipIfDatabaseNotInit(announceForMainPage.database)();
		...
		try{
			var AnnouncementModelList = await announceForMainPage.returnDataList(announceForMainPage.database.AnnouncementModel, options);
		}catch(ERROR){
			announceForMainPage.checkAndAlertIfError(ERROR)();
		}finally{
			res.send(AnnouncementModelList);
		}
	}else{
		res.send("");
	}
